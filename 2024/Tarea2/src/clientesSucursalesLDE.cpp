        ...
        if (n <= totalSucursales) {
            ...
            if (nodoActual != NULL) {
                ...
                if (nodoActual->ant != NULL) {
                    // Apuntamos el nodo anterior al 'actual' hacia el nodo siguiente al 'actual'
                    nodoActual->ant->sig = nodoActual->sig; 
                } 
                // Caso 2 - Es el primer nodo
                else {
                    // Actualizamos el comienzo de la lista
                    clientesSucursalesLDE->inicial = nodoActual->sig; 
                }
                
                // Caso 3 - No es el ultimo nodo
                if (nodoActual->sig != NULL) {
                    nodoActual->sig->ant = nodoActual->ant; 
                    // Apuntamos el nodo siguiente al 'actual' hacia el nodo anterior al 'actual'
                }
