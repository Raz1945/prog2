...
//* Inserta la promoción en la lista de promociones ordenada de menor a mayor por 'fecha de inicio' de la promoción.
//* En caso de empate la promoción 'se almacena antes' de las existentes con misma fecha.
...
//* El formato en el que se debe imprimir la lista es utilizando de forma secuencial la función imprimirTPromocion. 
//* Si la lista es vacía no imprime nada. 
//  La función es Theta(n*m) peor caso, siendo n la cantidad de promociones
...
//* Recibe una referencia a un elemento de tipo TListaPromociones y libera su memoria Si 'liberarPromociones' es true,
//* libera además la memoria de cada una de las promociones en la colección. 
//  La función es O(n) peor caso, siendo n la cantidad de promociones en la lista.
void liberarTListaPromociones(TListaPromociones &listaPromociones,
                              bool liberarPromociones);

// Funcion para verificar si una lista de promociones es vacia. 
// Retorna true si y solo si la lista es vacia. 
// La función es O(1) peor caso.
...
// Recibe una lista de promociones y un id de promoción y regresa un booleano indicando si pertenece a la lista . 
// La función es O(n) peor caso, siendo n la cantidad de promociones en la lista.
...
//  Recibe una lista de promociones y un id de promoción y regresa la promoción con ese id. 
//  La función es O(n) peor caso, siendo n la cantidad de promociones en la lista
//* PRE: La promoción pertenece a la lista.
TPromocion obtenerPromocionTListaPromociones(TListaPromociones listaPromociones,
                                             int idPromocion);

//   Dada una lista de promociones y una fecha, devuelve una nueva lista de promociones con las promociones que finalizaron. 
//* Remueve las promociones finalizadas de la lista parámetro. 
//   La función es O(n) peor caso, donde n es la cantidad de promociones de la lista.
TListaPromociones obtenerPromocionesFinalizadas(TListaPromociones &listaPromociones,
                                                TFecha fecha);

//  Dada una lista de promociones y una fecha, devuelve una nueva lista de promociones con las promociones que están activas.
//* Remueve las promociones activas de la lista parámetro. 
//  La función es O(n) peor caso, donde n es la cantidad de promociones de la lista.
TListaPromociones obtenerPromocionesActivas(TListaPromociones &listaPromociones,
                                            TFecha fecha);

// Dada una lista de promociones y una promocion, devuelve true si la promoción es compatible con todas las promociones de la lista. 
// La función es O(n) peor caso, donde n es la cantidad de promociones de la lista.
